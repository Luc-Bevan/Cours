...
var EstimationUtils = class {
   ...
   */
  static estimateReviewTime(file, fileContent, contentType = "notes") {
    if (!file) {
...
var DataStorage = class {
  ...
  recordReview(path, response, isSkipped = false) {
  ...
  scheduleSessionForReview(sessionId) {
...
var MCQS = "MCQs";
var API = "API";
var SPACEFORGE = "Spaceforge";
var CLAUDE = "Claude";
var GEMINI = "Gemini";
var OLLAMA = "Ollama";
var OPENAI = "OpenAI";
var OPENAI_COMPATIBLE = "OpenAI Compatible";
var TOGETHER_AI = "Together AI";
...
var ReviewModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    postponeButton.addEventListener("click", () => {
      void this.plugin.reviewController.skipReview(this.path);
      this.close();
    });
    ...
    if (this.plugin.settings.enableMCQ) {
      ...
      mcqButton.addEventListener("click", () => {
        ...
        if (mcqController2) {
          void mcqController2.startMCQReview(this.path);
          this.close();
        } else {
          ...
          if (initializedMcqController) {
            ...
            new import_obsidian2.Notice("Feature could not be initialized. Please check settings.");
          }
        }
      });
...
var ReviewControllerCore = class {
   ...
   */
  updateTodayNotes(preserveCurrentIndex = false) {
  ...
  async postponeNote(path, days = 1) {
    var _a;
    this.plugin.dataStorage.reviewScheduleService.postponeNote(path, days);
    await this.plugin.savePluginData();
    this.handleNotePostponed(path);
    void ((_a = this.plugin.getSidebarView()) == null ? void 0 : _a.refresh());
  }
  /**
  ...
  async skipReview(path) {
    var _a;
    const effectiveDate = this.getEffectiveReviewDate();
    this.plugin.dataStorage.reviewScheduleService.skipNote(path, 3 /* CorrectWithDifficulty */, effectiveDate);
    await this.plugin.savePluginData();
...
var ConsolidatedMCQModal = class extends import_obsidian5.Modal {
  ...
  displayCurrentQuestion(containerEl) {
    ...
    if (!question || !question.choices || question.choices.length < 2) {
      new import_obsidian5.Notice("Invalid question data. Moving to next question.");
  ...
  completeReview() {
    ...
    for (const notePath in noteScores) {
      ...
      let success = false;
      let response = 1 /* IncorrectResponse */;
      if (score >= 0.9) {
        success = true;
        response = 5 /* PerfectRecall */;
      } else if (score >= 0.7) {
        success = true;
        response = 4 /* CorrectWithHesitation */;
      } else if (score >= 0.5) {
        success = true;
        response = 3 /* CorrectWithDifficulty */;
      } else {
        success = false;
    ...
    const { contentEl } = this;
    contentEl.empty();
    new import_obsidian5.Setting(contentEl).setName("Review complete").setHeading().setClass("mcq-review-complete-header");
    ...
    new import_obsidian5.Setting(noteScoresEl).setHeading().setName("Scores by note");
    ...
    new import_obsidian5.Setting(breakdownContainer).setHeading().setName("Detailed question breakdown");
...
var BatchReviewModal = class extends import_obsidian6.Modal {
  ...
  renderStartScreen(contentEl) {
    ...
    void this.estimateAndShowTime(infoEl);
  ...
  showConsolidatedMCQUI() {
    ...
    if (this.plugin.mcqController) {
      try {
        ...
        new import_obsidian6.Notice("Error showing review. Falling back to manual review.");
        this.open();
        void this.processNextManual();
      }
    } else {
      new import_obsidian6.Notice("Controller not available. Falling back to manual review.");
      this.open();
      void this.processNextManual();
    }
  }
  recordAllReviews(results) {
  ...
  async processNextMCQ() {
    ...
    const { contentEl } = this;
    contentEl.empty();
    new import_obsidian6.Setting(contentEl).setName("Review in progress").setHeading();
    const progressEl = contentEl.createDiv("batch-review-progress");
    ...
    if (this.plugin.mcqController) {
      ...
      if (mcqSet) {
        void this.plugin.mcqController.startMCQReview(
          note.path
          /* Removed callback */
        );
        this.open();
  ...
  async processNextManual() {
    ...
    blackoutButton.addEventListener("click", () => void this.recordManualResult(note.path, 0 /* CompleteBlackout */));
    ...
    incorrectButton.addEventListener("click", () => void this.recordManualResult(note.path, 1 /* IncorrectResponse */));
    ...
    incorrectFamiliarButton.addEventListener("click", () => void this.recordManualResult(note.path, 2 /* IncorrectButFamiliar */));
    ...
    correctDifficultyButton.addEventListener("click", () => void this.recordManualResult(note.path, 3 /* CorrectWithDifficulty */));
    ...
    correctHesitationButton.addEventListener("click", () => void this.recordManualResult(note.path, 4 /* CorrectWithHesitation */));
    ...
    perfectRecallButton.addEventListener("click", () => void this.recordManualResult(note.path, 5 /* PerfectRecall */));
  ...
  recordManualResult(path, response) {
  ...
  showSummary() {
    const { contentEl } = this;
    contentEl.empty();
    new import_obsidian6.Setting(contentEl).setName("Review complete").setHeading().setClass("mcq-review-complete-header");
    ...
    new import_obsidian6.Setting(resultsEl).setHeading().setName("Individual results");
...
var ReviewBatchController = class {
   ...
   */
  reviewNotes(paths, useMCQ = false) {
    if (paths.length === 0) {
   ...
   */
  reviewAllNotesWithMCQ(useMCQ = true) {
    const reviewController = this.plugin.reviewController;
    if (!reviewController)
      return;
    const hasCustomOrder = this.plugin.reviewScheduleService.customNoteOrder.length > 0;
  ...
  async regenerateAllMCQs() {
    ...
    if (!this.plugin.mcqController) {
      new import_obsidian7.Notice("Questions feature not initialized. Check settings.");
      return;
    }
...
var MCQModal = class extends import_obsidian8.Modal {
      ...
      refreshBtn.addEventListener("click", () => {
        ...
        void (async () => {
          ...
          if (mcqGenerationService && this.plugin.mcqController) {
            ...
            new import_obsidian8.Notice("Generation service not available.");
          }
        })();
      });
    }
  ...
  displayCurrentQuestion(containerEl) {
    ...
    const question = this.mcqSet.questions[questionIndex];
    if (!question || !question.choices || question.choices.length < 2) {
      new import_obsidian8.Notice("Invalid question data. Moving to next question.");
    ...
    if (existingAnswer && existingAnswer.attempts >= 2) {
      ...
      skipButton.addEventListener("click", () => {
        ...
        if (choicesContainer2 instanceof HTMLElement)
          choicesContainer2.classList.add("sf-hidden");
        skipButton.classList.add("sf-hidden");
      });
    }
    const choicesContainer = newQuestionContainer.createDiv("mcq-choices-container");
    question.choices.forEach((choice, index) => {
  ...
  completeSession() {
    ...
    try {
      ...
      new import_obsidian8.Setting(resultsEl).setHeading().setName("Question results");
...
var MCQController = class {
  ...
  async startMCQReview(notePath, externalOnCompleteCallback) {
    if (!this.plugin.settings.enableMCQ) {
      new import_obsidian9.Notice("Feature is disabled in settings.");
      if (externalOnCompleteCallback)
        externalOnCompleteCallback(notePath, false);
      return;
    }
    if (!this.mcqGenerationService) {
      new import_obsidian9.Notice("Generation service is not available. Check API provider settings.");
      return;
    }
    try {
      let mcqSet = this.mcqService.getMCQSetForNote(notePath);
      if (mcqSet && mcqSet.needsQuestionRegeneration) {
        new import_obsidian9.Notice("Questions flagged for regeneration. Generating new set...");
        ...
        if (mcqSet) {
          ...
          new import_obsidian9.Notice("Failed to regenerate. Using existing set if available.");
          mcqSet = this.mcqService.getMCQSetForNote(notePath);
        }
      }
      if (!mcqSet || mcqSet.questions.length === 0) {
        new import_obsidian9.Notice("No questions found. Generating new set...");
        mcqSet = await this.generateMCQs(notePath);
        if (!mcqSet) {
          new import_obsidian9.Notice("Failed to generate questions.");
          return;
        }
      }
      const internalOnComplete = (path, score, completed) => {
        void (async () => {
          if (completed) {
      ...
      new import_obsidian9.Notice("Error starting review. Please check console for details.");
      if (externalOnCompleteCallback)
        externalOnCompleteCallback(notePath, false);
    }
  }
  ...
  async generateMCQs(notePath, forceRegeneration = false) {
    if (!this.plugin.settings.enableMCQ || !this.mcqGenerationService) {
      new import_obsidian9.Notice("Feature is disabled or the generation service is not available. Check API provider settings.");
      return null;
    }
  ...
  async startConsolidatedMCQReviewForSelectedDate() {
    ...
    new import_obsidian9.Notice(`Fetching questions for ${dueNotes.length} due note(s)...`);
    for (const noteSchedule of dueNotes) {
      const notePath = noteSchedule.path;
      let mcqSet = this.mcqService.getMCQSetForNote(notePath);
      if (mcqSet && mcqSet.needsQuestionRegeneration) {
        new import_obsidian9.Notice(`Regenerating flagged questions for ${notePath}...`);
        ...
        if (regeneratedMcqSet) {
          ...
          new import_obsidian9.Notice(`Failed to regenerate questions for ${notePath}. Using existing set if available (might be outdated or empty).`);
        }
      }
      if (!mcqSet || mcqSet.questions.length === 0) {
        new import_obsidian9.Notice(`No questions found or set is empty for ${notePath}. Attempting to generate new set...`);
        const newMcqSet = await this.generateMCQs(notePath, false);
        if (newMcqSet) {
          mcqSet = newMcqSet;
        } else {
          new import_obsidian9.Notice(`Failed to generate questions for ${notePath}. This note will be skipped.`);
        }
      }
      if (mcqSet && mcqSet.questions.length > 0) {
        const file = this.plugin.app.vault.getAbstractFileByPath(notePath);
    ...
    if (mcqSetsForReview.length === 0) {
      new import_obsidian9.Notice("No questions available for the due notes.");
      return;
    }
    await this.plugin.savePluginData();
    new import_obsidian9.Notice(`Starting consolidated review for ${notesWithMCQsCount} note(s) with ${mcqSetsForReview.reduce((sum, s) => sum + s.mcqSet.questions.length, 0)} questions.`);
    const onConsolidatedComplete = (results) => {
      void (async () => {
        ...
        if (reviewsProcessed > 0) {
          ...
          new import_obsidian9.Notice("No reviews updated.");
        }
      })();
    };
...
var PomodoroUIManager = class {
  ...
  _savePomodoroSettings() {
    ...
    if (work > 0 && short > 0 && long > 0 && sessions > 0) {
      ...
      new import_obsidian12.Notice("Invalid durations. Settings not saved. Please enter positive numbers.");
    ...
    if (this.pomodoroTimerDisplayEl) {
      ...
      const handlePressEnd = () => {
        if (this.veryLongPressTimer)
          window.clearTimeout(this.veryLongPressTimer);
  ...
  updateButtonVisibility() {
    ...
    if (this.pomodoroStartBtn)
      this.pomodoroStartBtn.toggleClass("sf-hidden", isRunning);
    if (this.pomodoroStopBtn)
      this.pomodoroStopBtn.toggleClass("sf-hidden", !isRunning);
    if (this.pomodoroSkipBtn)
      this.pomodoroSkipBtn.toggleClass("sf-hidden", !this.areButtonsVisible);
  }
  /**
   * Updates the Pomodoro UI based on the current state from PomodoroService.
   */
...
var DateUtils = class {
  ...
  static formatDate(timestamp, format = "medium", baseDateParam) {
    ...
    if (format === "relative") {
      ...
      const normalizedActualCurrentDate = this.startOfUTCDay(/* @__PURE__ */ new Date());
      if (normalizedNoteEventDate < normalizedActualCurrentDate) {
        return "Due notes";
      }
      if (baseDateParam) {
        const normalizedBaseDate = this.startOfUTCDay(new Date(baseDateParam));
...
var ConfirmationModal = class extends import_obsidian13.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian13.Setting(buttonContainer).addButton((btn) => btn.setButtonText("Cancel").onClick(() => {
      void this.onCancel();
      this.close();
...
var NoteItemRenderer = class {
  ...
  async _populateNoteItemDetails(noteEl, note, dateStr, selectedNotesArray) {
    ...
    if (dateStr === "Due notes") {
      ...
      const daysOverdue = Math.abs(Math.floor((note.nextReviewDate - DateUtils.startOfUTCDay()) / (24 * 60 * 60 * 1e3)));
    ...
    if (isRecurring) {
      ...
      if (recurrenceInfo) {
        ...
        recurrenceBadge.title = `This note recurs every ${recurrenceInfo.interval} days`;
      }
    } else {
    ...
    if (titleEl) {
      const file = this.plugin.app.vault.getAbstractFileByPath(note.path);
      titleEl.setText(file instanceof import_obsidian14.TFile ? file.basename : note.path);
    }
    ...
    const dragHandleEl = buttonsEl == null ? void 0 : buttonsEl.querySelector(".review-note-drag-handle");
    if (dragHandleEl) {
    ...
    if (advanceBtn) {
      const todayStartTs = DateUtils.startOfUTCDay(/* @__PURE__ */ new Date());
      const noteReviewDayStartTs = DateUtils.startOfUTCDay(new Date(note.nextReviewDate));
  ...
  async renderNoteItem(notesContainer, noteToRender, dateStr, parentContainerForBulkActions, selectedNotesArray, lastSelectedNotePathRef, onSelectionChange, onNoteAction) {
    ...
    (0, import_obsidian14.setIcon)(recurrenceBtn, "repeat");
    ...
    titleEl.addEventListener("click", (e) => {
      ...
      if (path)
        void this.plugin.reviewController.openNoteWithoutReview(path);
    });
      ...
      if (path) {
        void (async () => {
          try {
            ...
            new import_obsidian14.Notice(`Note postponed`);
            await onNoteAction();
          } catch (_error) {
            new import_obsidian14.Notice("Failed to postpone note.");
            await onNoteAction();
          }
        })();
      }
    ...
    removeBtn.addEventListener("click", (e) => {
      ...
      if (path) {
        ...
        new ConfirmationModal(
          this.plugin.app,
          "Remove Note",
          `Remove "${file instanceof import_obsidian14.TFile ? file.basename : path}" from review schedule?`,
          () => {
            void (async () => {
              try {
                this.plugin.reviewScheduleService.removeFromReview(path);
                await this.plugin.savePluginData();
                new import_obsidian14.Notice(`Note removed from review schedule`);
                await onNoteAction();
              } catch (_error) {
                new import_obsidian14.Notice("Failed to remove note from schedule.");
                await onNoteAction();
              }
            })();
          }
      ...
      if (path) {
        void (async () => {
          ...
          if (isCurrentlyRecurring) {
            ...
            new import_obsidian14.Notice(`Note is no longer recurring`);
          } else {
            ...
            new import_obsidian14.Notice(`Note is now recurring every ${defaultInterval} days`);
          }
          await onNoteAction();
        })();
      }
    ...
    noteEl.addEventListener("contextmenu", (e) => {
      ...
      menu.addItem((item) => item.setTitle("Review note").setIcon("play-circle").onClick(() => {
      ...
      const schedule = this.plugin.reviewScheduleService.schedules[path];
      if (schedule) {
        const todayStart = DateUtils.startOfUTCDay(/* @__PURE__ */ new Date());
        const noteReviewDayStart = DateUtils.startOfUTCDay(new Date(schedule.nextReviewDate));
      ...
      const isCurrentlyRecurring = this.plugin.recurringNotesService.isNoteRecurring(path);
      if (isCurrentlyRecurring) {
        menu.addItem((item) => item.setTitle("Stop recurrence").setIcon("x-circle").onClick(async () => {
          await this.plugin.recurringNotesService.convertToRegularNote(path);
          new import_obsidian14.Notice("Note is no longer recurring");
          await onNoteAction();
        }));
        menu.addItem((item) => item.setTitle("Change recurrence interval").setIcon("calendar-range").onClick(async () => {
          ...
          if (recurrenceInfo) {
            ...
            new import_obsidian14.Notice(`Recurrence interval changed to ${newInterval} days`);
            await onNoteAction();
          }
        }));
      } else {
        menu.addItem((item) => item.setTitle("Make recurring").setIcon("repeat").onClick(async () => {
          const defaultInterval = this.plugin.settings.defaultRecurrenceInterval;
          await this.plugin.recurringNotesService.convertToRecurringNote(path, defaultInterval);
          new import_obsidian14.Notice(`Note is now recurring every ${defaultInterval} days`);
          await onNoteAction();
      ...
      menu.addItem((item) => item.setTitle("Remove from review").setIcon("trash").onClick(() => {
        const file = this.plugin.app.vault.getAbstractFileByPath(path);
        new ConfirmationModal(
          this.plugin.app,
          "Remove Note",
...
var ListViewRenderer = class {
  ...
  // }
  _ensureAndUpdateReviewButtonsSection(container, notesForDisplay, _selectedNotes) {
    ...
    let pomodoroSectionContainerEl = pomodoroContainer.querySelector(".sidebar-pomodoro-section-container");
    if (!pomodoroSectionContainerEl) {
      const legacyEl = pomodoroContainer.querySelector(".pomodoro-section-container, .sidebar-pomodoro-button-container");
      if (legacyEl) {
    ...
    if (notesForDisplay.length > 0) {
      if (!reviewButtonsContainer) {
        ...
        prevNoteBtn.addEventListener("click", () => {
          void this.plugin.reviewController.navigateToPreviousNote();
        });
        ...
        nextNoteBtn.addEventListener("click", () => {
          void this.plugin.reviewController.navigateToNextNote();
        });
        ...
        reviewCurrentBtn.addEventListener("click", () => {
          void this.plugin.reviewController.reviewCurrentNote();
        });
        ...
        reviewAllBtn.addEventListener("click", () => {
          void this.plugin.reviewController.reviewAllTodaysNotes();
        });
        if (this.plugin.settings.enableMCQ) {
      ...
      if (!bulkActionButtons) {
        ...
        reviewSelectedBtn.addEventListener("click", () => {
          void this.plugin.reviewController.reviewNotes(this.getSelectedNotes(), false);
        ...
        advanceSelectedBtn.addEventListener("click", () => {
          ...
          if (pathsToAdvance.length === 0) {
            new import_obsidian15.Notice("No notes selected to advance.");
            return;
          }
          void (async () => {
        ...
        postponeSelectedBtn.addEventListener("click", () => {
          ...
          if (pathsToPostpone.length === 0) {
            new import_obsidian15.Notice("No notes selected to postpone.");
            return;
          }
          this.setSelectedNotes([]);
          void (async () => {
          ...
          new ConfirmationModal(
            ...
            () => {
              ...
              void (async () => {
                ...
                await this.plugin.savePluginData();
                await this.refreshSidebarView();
                new import_obsidian15.Notice(`Removed ${pathsToRemove.length} selected notes.`);
              })();
            }
          ).open();
        });
      ...
      if (bulkActionButtons)
        bulkActionButtons.toggleClass("sf-hidden", true);
    }
  }
  ...
  async _ensureAndUpdateDateSections(container, sortedDateKeys, groupedNotes) {
    ...
    for (const dateStr of sortedDateKeys) {
      ...
      if (!dateSectionEl) {
        ...
        const todayStart = DateUtils.startOfUTCDay(/* @__PURE__ */ new Date());
        const sectionDateKeyIsFuture = !["Due notes", "Today"].includes(dateStr) && (notesForSection[0] && DateUtils.startOfUTCDay(new Date(notesForSection[0].nextReviewDate)) > todayStart);
      ...
      const dateHeading = headerContainer == null ? void 0 : headerContainer.querySelector("h3");
      const reviewTimeEl = dateSectionEl.querySelector(".review-date-time");
      if (!headerContainer || !dateHeading)
        continue;
      ...
      const todayActualStart = DateUtils.startOfUTCDay();
      ...
      if (shouldShowOverdueBadge) {
        ...
        if (overdueNotesInThisSection.length > 0) {
          ...
          if (maxDays > 0) {
            ...
            overdueBadge.toggleClass("sf-hidden", true);
          }
        } else if (overdueBadge) {
          overdueBadge.toggleClass("sf-hidden", true);
        }
  ...
  _ensureAndUpdateActiveSessionSection(container) {
    ...
    if (activeSession) {
      if (!sessionSection) {
        ...
        new import_obsidian15.Setting(sessionSection).setHeading().setName("Active review session");
      ...
      const nameEl = sessionSection.querySelector(".review-session-name");
      if (nameEl)
        nameEl.setText(activeSession.name);
      const progressEl = sessionSection.querySelector(".review-session-progress");
      if (progressEl)
        progressEl.setText(`Progress: ${activeSession.currentIndex}/${activeSession.hierarchy.traversalOrder.length}`);
  ...
  async _ensureAndUpdateUpcomingReviewsSection(container) {
    ...
    const upcomingKeys = this.getSortedDateKeys(upcomingGroupedNotes).filter((key) => {
      if (key === "Due notes")
        return false;
      const actualTodayStart = DateUtils.startOfUTCDay(/* @__PURE__ */ new Date());
    ...
    if (upcomingKeys.length > 0) {
      if (!upcomingSection) {
        ...
        new import_obsidian15.Setting(upcomingSection).setHeading().setName("Upcoming reviews");
      ...
      for (const dayKey of upcomingKeys) {
        ...
        if (!dayItemEl) {
          ...
          daySummary.createSpan({ cls: "review-upcoming-day-name" });
          dayItemEl.addEventListener("click", (e) => {
            const currentDayKey = e.currentTarget.dataset.dayKey;
            if (!currentDayKey)
              return;
        ...
        if (isExpanded) {
          ...
          notesContainerEl.toggleClass("sf-hidden", true);
        }
      }
  ...
  * (Called by handleSelectionChange)
  */
  async updateSelectionInfo(container) {
    ...
    if (bulkActionsContainer && selectedNotesPaths.length > 0) {
      let selectionInfo = bulkActionsContainer.querySelector(".review-selection-info");
      if (!selectionInfo) {
    ...
    if (bulkActionsContainer) {
      ...
      if (advanceSelectedBtn) {
        if (selectedNotesPaths.length > 1) {
          ...
          advanceSelectedBtn.toggleClass("sf-hidden", false);
        } else {
  ...
  setupDragAndDrop(container) {
    ...
    container.addEventListener("drop", (e) => {
      ...
      void (async () => {
        var _a;
        const draggedPath = (_a = e.dataTransfer) == null ? void 0 : _a.getData("text/plain");
        if (!draggedPath)
          return;
        const target = e.target;
        const noteItem = target.closest(".review-note-item");
        if (!noteItem)
          return;
  ...
  async updateHeaderStats(container) {
    ...
    if (reviewButtonsContainer) {
      reviewButtonsContainer.toggleClass("sf-hidden", dueNotesForStats.length === 0);
    }
    this.updateBulkActionButtonsVisibility(container);
    ...
    if (allCaughtUpEl) {
      allCaughtUpEl.toggleClass("sf-hidden", notesInCurrentContext.length !== 0);
      ...
      if (headerTextEl && timeEl && dateStr) {
        ...
        if (dateStr === "Due notes") {
          const daysDiff = notesInSection.map((noteEl) => {
            ...
            return noteSchedule ? Math.abs(Math.floor((noteSchedule.nextReviewDate - DateUtils.startOfUTCDay()) / (24 * 60 * 60 * 1e3))) : 0;
          });
  ...
  */
   ...
   */
  groupNotesByDate(notes, _includeFuture = false) {
    const grouped = {};
    const actualTodayStart = DateUtils.startOfUTCDay(/* @__PURE__ */ new Date());
    const activeListBaseDate = this.getActiveListBaseDate();
    ...
    for (const note of notes) {
      ...
      const noteDateStart = DateUtils.startOfUTCDay(noteDate);
      if (activeListBaseDate) {
...
var UpcomingEvents = class {
  ...
  renderEmptyState() {
    ...
    (0, import_obsidian16.setIcon)(emptyIcon, "calendar");
  ...
  renderEventItem(container, upcomingEvent) {
    ...
    if (event.description || event.location) {
      ...
      if (event.location) {
        ...
        (0, import_obsidian16.setIcon)(eventLocation, "map-pin");
...
\u{1F3F7}\uFE0F ${event.category}`;
    new import_obsidian16.Notice(details, 8e3);
  }
  /**
...
// ui/event-modal.ts
var EventModal = class extends import_obsidian17.Modal {
  ...
  onOpen() {
    ...
    (0, import_obsidian17.setIcon)(headerIcon, this.event ? "edit" : "calendar-plus");
    const headerText = header.createDiv("event-modal-header-text");
    const titleSetting = new import_obsidian17.Setting(headerText).setHeading().setName(this.event ? "Edit event" : "New event");
  ...
  async deleteEvent() {
    ...
    try {
      ...
      if (deleted) {
        new import_obsidian17.Notice("Event deleted successfully");
        await this.plugin.savePluginData();
        this.onSave(this.event);
        this.close();
      } else {
        new import_obsidian17.Notice("Failed to delete event");
      }
    } catch (error) {
      console.error("Error deleting event:", error);
      new import_obsidian17.Notice("Error deleting event");
    }
  }
  /**
   * Save the event
  ...
  async saveEvent() {
    ...
    try {
      ...
      if (!title || !this.dateInput.value) {
        new import_obsidian17.Notice("Please fill in all required fields");
        return;
      }
      let eventData;
      if (this.event) {
        ...
        if (updatedEvent) {
          ...
          new import_obsidian17.Notice("Event updated successfully");
        }
      } else {
        eventData = {
          title,
        ...
        if (newEvent) {
          ...
          new import_obsidian17.Notice("Event created successfully");
        }
      }
      await this.plugin.savePluginData();
      this.close();
      ...
      new import_obsidian17.Notice("Error saving event");
    }
  }
};
...
var CalendarView = class {
  ...
  ensureCalendarBaseStructure() {
    ...
    if (!this.calendarHeaderEl || !calendarContainer.contains(this.calendarHeaderEl)) {
      ...
      (0, import_obsidian18.setIcon)(prevMonthBtn, "chevron-left");
      ...
      (0, import_obsidian18.setIcon)(nextMonthBtn, "chevron-right");
      ...
      (0, import_obsidian18.setIcon)(addEventBtn, "plus");
          ...
          dayCell.addEventListener("click", () => {
            ...
            void (async () => {
              ...
              if (sidebarView && typeof sidebarView.refresh === "function") {
                ...
                new import_obsidian18.Notice("Switched to list view. Sidebar will update.");
              }
            })();
          });
...
\u{1F4CD} ${event.location} `;
    ...
    details += `
\u{1F3F7}\uFE0F ${event.category} `;
    new import_obsidian18.Notice(details, 8e3);
  }
  /**
...
// ui/sidebar-view.ts
var ReviewSidebarView = class extends import_obsidian19.ItemView {
  ...
  async onOpen() {
    this.plugin.events.on("sidebar-update", () => {
      void this.refresh();
    });
  ...
  onClose() {
    ...
    this.plugin.events.off("sidebar-update", () => {
      void this.refresh();
    });
  ...
  ensureBaseStructure() {
    ...
    if (!this.persistentHeaderEl) {
      ...
      new import_obsidian19.Setting(this.persistentHeaderEl).setHeading().setName("Review schedule");
  ...
  async moveNoteUp(dateStr, note) {
    ...
    await this.refresh();
    new import_obsidian19.Notice(`Moved note up.`);
  }
  /**
  ...
  async moveNoteDown(dateStr, note) {
    ...
    await this.plugin.reviewController.swapNotes(path1, path2);
    await this.refresh();
    new import_obsidian19.Notice(`Moved note down.`);
  }
  /**
...
// ui/settings-tab.ts
var SpaceforgeSettingTab = class extends import_obsidian20.PluginSettingTab {
  /**
  ...
  display() {
    ...
    const createCollapsible = (title, iconName, defaultOpen = true) => {
      ...
      const headingSetting = new import_obsidian20.Setting(sectionContainer).setName(title).setHeading();
      if (iconName) {
    ...
    const createActionButtons = () => {
      ...
      exportBtn.addEventListener("click", () => {
        ...
        new import_obsidian20.Notice("All data exported successfully");
      });
        ...
        input.onchange = async (e) => {
          ...
          if (file) {
            try {
              ...
              await this.plugin.savePluginData();
              this.display();
              new import_obsidian20.Notice("All data imported successfully. Plugin may require a reload for all changes to take effect.");
            } catch (error) {
              const msg = error instanceof Error ? error.message : String(error);
              new import_obsidian20.Notice(`Failed to import data: ${msg} `);
            }
          }
        };
        input.click();
      ...
      resetBtn.addEventListener("click", () => {
        new ConfirmationModal(
          ...
          async () => {
            ...
            await this.plugin.savePluginData();
            this.display();
            new import_obsidian20.Notice("All data reset to defaults.");
          }
        ).open();
      });
        ...
        new ConfirmationModal(
          ...
          async () => {
            ...
            try {
              ...
              await this.plugin.savePluginData();
              new import_obsidian20.Notice("All review data cleared successfully.");
              this.display();
              void ((_a = this.plugin.getSidebarView()) == null ? void 0 : _a.refresh());
            } catch (e) {
              new import_obsidian20.Notice("Failed to clear review data. Check console for details.");
            }
          }
        ).open();
      });
        ...
        new ConfirmationModal(
          ...
          async () => {
            ...
            try {
              ...
              await this.plugin.savePluginData();
              new import_obsidian20.Notice("All calendar events cleared successfully.");
              this.display();
              void ((_a = this.plugin.getSidebarView()) == null ? void 0 : _a.refresh());
            } catch (e) {
              new import_obsidian20.Notice("Failed to clear calendar events. Check console for details.");
            }
          }
        ).open();
      });
        ...
        new ConfirmationModal(
          ...
          async () => {
            ...
            try {
              ...
              await this.plugin.savePluginData();
              new import_obsidian20.Notice("All data cleared successfully.");
              this.display();
              void ((_d = this.plugin.getSidebarView()) == null ? void 0 : _d.refresh());
            } catch (e) {
              new import_obsidian20.Notice("Failed to clear all data. Check console for details.");
            }
          }
        ).open();
      });
    ...
    new import_obsidian20.Setting(spacedRepSection).setName("Algorithm configuration").setHeading();
    const algorithmContainer = spacedRepSection.createDiv({ cls: "sf-settings-subsection" });
    new import_obsidian20.Setting(algorithmContainer).setName("Default scheduling algorithm").setDesc("Choose the default algorithm for newly created notes.").addDropdown((dropdown) => dropdown.addOption("sm2", SM2).addOption("fsrs", FSRS).setValue(this.plugin.settings.defaultSchedulingAlgorithm).onChange(async (value) => {
    ...
    new import_obsidian20.Setting(sm2ParamsContainer).setName(`Base ease factor (${SM2})`).setDesc(`Initial ease factor for new ${SM2} notes (2.5 is ${SM2} default). Higher ease increases interval growth. Value shown is internal format (250 = 2.5).`).addSlider((slider) => slider.setLimits(130, 500, 10).setValue(this.plugin.settings.baseEase).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.baseEase = value;
      await this.plugin.savePluginData();
    }));
    new import_obsidian20.Setting(sm2ParamsContainer).setName(`Use initial learning schedule (${SM2})`).setDesc(`For new ${SM2} notes, use a fixed set of initial intervals before applying the full algorithm.`).addToggle((toggle) => toggle.setValue(this.plugin.settings.useInitialSchedule).onChange(async (value) => {
      this.plugin.settings.useInitialSchedule = value;
      await this.plugin.savePluginData();
      this.display();
    }));
    if (this.plugin.settings.useInitialSchedule) {
      new import_obsidian20.Setting(sm2ParamsContainer).setName(`Custom initial intervals (${SM2})`).setDesc(`Comma-separated list for initial ${SM2} reviews (e.g., 0,1,3,7). Must start with 0.`).addText((text) => text.setValue(this.plugin.settings.initialScheduleCustomIntervals.join(", ")).onChange(async (value) => {
        ...
        if (intervals.length > 0 && intervals[0] === 0) {
          ...
          new import_obsidian20.Notice(`Custom initial ${SM2} intervals must start with 0 and be valid numbers.`, 5e3);
          text.setValue(this.plugin.settings.initialScheduleCustomIntervals.join(", "));
        }
      }));
    }
    new import_obsidian20.Setting(sm2ParamsContainer).setName(`Maximum interval (${SM2})`).setDesc(`Longest possible interval between ${SM2} reviews.`).addText((text) => text.setValue(this.plugin.settings.maximumInterval.toString()).onChange(async (value) => {
    ...
    new import_obsidian20.Setting(sm2ParamsContainer).setName(`Load balancing (${SM2})`).setDesc(`Add slight randomness to ${SM2} intervals to prevent reviews clumping on the same day.`).addToggle((toggle) => toggle.setValue(this.plugin.settings.loadBalance).onChange(async (value) => {
    ...
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Request retention").setDesc("Desired recall probability (0.7-0.99, default: 0.9). Higher values mean more frequent reviews.").addText((text) => {
      var _a, _b, _c, _d, _e;
      return text.setValue((_e = (_d = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.request_retention) == null ? void 0 : _b.toString()) != null ? _d : (_c = DEFAULT_SETTINGS.fsrsParameters.request_retention) == null ? void 0 : _c.toString()) != null ? _e : "0.9").onChange(async (value) => {
        ...
        if (!isNaN(numValue) && numValue >= 0.7 && numValue <= 0.99) {
          ...
          new import_obsidian20.Notice(`${FSRS} request retention must be between 0.7 and 0.99.`);
        }
      });
    });
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Maximum interval (days)").setDesc(`Longest possible interval ${FSRS} will schedule.`).addText((text) => {
      var _a, _b, _c, _d, _e;
      return text.setValue((_e = (_d = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.maximum_interval) == null ? void 0 : _b.toString()) != null ? _d : (_c = DEFAULT_SETTINGS.fsrsParameters.maximum_interval) == null ? void 0 : _c.toString()) != null ? _e : "36500").onChange(async (value) => {
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
          ...
          new import_obsidian20.Notice(`${FSRS} maximum interval must be a positive number.`);
        }
      });
    });
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Learning steps (minutes)").setDesc("Comma-separated initial learning intervals in minutes (e.g., 1,10 for 1m, 10m).").addText((text) => {
      var _a, _b, _c, _d, _e;
      return text.setValue((_e = (_d = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.learning_steps) == null ? void 0 : _b.join(",")) != null ? _d : (_c = DEFAULT_SETTINGS.fsrsParameters.learning_steps) == null ? void 0 : _c.join(",")) != null ? _e : "").onChange(async (value) => {
        ...
        if (steps.length > 0) {
          ...
          new import_obsidian20.Notice(`${FSRS} learning steps must be valid comma-separated numbers > 0, or empty.`);
        }
      });
    });
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Enable fuzz").setDesc(`Add slight randomness to ${FSRS} intervals (recommended).`).addToggle((toggle) => {
      var _a, _b, _c;
      return toggle.setValue((_c = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.enable_fuzz) != null ? _b : DEFAULT_SETTINGS.fsrsParameters.enable_fuzz) != null ? _c : false).onChange(async (value) => {
        this.plugin.settings.fsrsParameters = { ...this.plugin.settings.fsrsParameters, enable_fuzz: value };
        await this.plugin.savePluginData();
        this.plugin.reviewScheduleService.updateAlgorithmServicesForSettingsChange();
      });
    });
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Enable short-term scheduling").setDesc(`Use ${FSRS} short-term memory model (affects initial learning steps).`).addToggle((toggle) => {
      var _a, _b, _c;
      return toggle.setValue((_c = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.enable_short_term) != null ? _b : DEFAULT_SETTINGS.fsrsParameters.enable_short_term) != null ? _c : false).onChange(async (value) => {
        this.plugin.settings.fsrsParameters = { ...this.plugin.settings.fsrsParameters, enable_short_term: value };
        await this.plugin.savePluginData();
        this.plugin.reviewScheduleService.updateAlgorithmServicesForSettingsChange();
      });
    });
    new import_obsidian20.Setting(fsrsParamsContainer).setName("Weights (w)").setDesc(`${FSRS} algorithm parameters (17 numbers). Edit with caution. Default weights are generally good.`).addTextArea((text) => {
      var _a, _b, _c, _d, _e;
      text.inputEl.rows = 3;
      text.inputEl.addClass("sf-full-width-textarea");
      text.setValue((_e = (_d = (_b = (_a = this.plugin.settings.fsrsParameters) == null ? void 0 : _a.w) == null ? void 0 : _b.join(",")) != null ? _d : (_c = DEFAULT_SETTINGS.fsrsParameters.w) == null ? void 0 : _c.join(",")) != null ? _e : "").onChange(async (value) => {
        ...
        if (weights.length === 17 && weights.every((n) => !isNaN(n))) {
          ...
          new import_obsidian20.Notice(`${FSRS} weights must be a comma-separated list of 17 valid numbers.`);
        }
      });
    });
    ...
    new import_obsidian20.Setting(conversionContainer).setName(`Convert all ${SM2} cards to ${FSRS}`).setDesc(`Migrate all existing ${SM2} cards to use the ${FSRS} algorithm. This will reset their learning state for ${FSRS}.`).addButton((button) => button.setButtonText(`Convert ${SM2} to ${FSRS}`).setCta().onClick(() => {
      new ConfirmationModal(
        this.app,
        `Convert ${SM2} to ${FSRS}`,
        ...
        async () => {
          new import_obsidian20.Notice(`Converting ${SM2} cards to ${FSRS}... This may take a moment.`);
          this.plugin.reviewScheduleService.convertAllSm2ToFsrs();
          await this.plugin.savePluginData();
          new import_obsidian20.Notice(`All ${SM2} cards have been converted to ${FSRS}.`);
          this.display();
        }
      ).open();
    }));
    new import_obsidian20.Setting(conversionContainer).setName(`Convert all ${FSRS} cards to ${SM2}`).setDesc(`Migrate all existing ${FSRS} cards to use the ${SM2} algorithm. Their ${SM2} learning state will be initialized with defaults.`).addButton((button) => button.setButtonText(`Convert ${FSRS} to ${SM2}`).setCta().onClick(() => {
      new ConfirmationModal(
        this.app,
        `Convert ${FSRS} to ${SM2}`,
        ...
        async () => {
          new import_obsidian20.Notice(`Converting ${FSRS} cards to ${SM2}... This may take a moment.`);
          this.plugin.reviewScheduleService.convertAllFsrsToSm2();
          await this.plugin.savePluginData();
          new import_obsidian20.Notice(`All ${FSRS} cards have been converted to ${SM2}.`);
          this.display();
        }
      ).open();
    }));
    const interfaceSection = createCollapsible("Interface & behavior", "settings", false);
    new import_obsidian20.Setting(interfaceSection).setName("Display").setHeading().setClass("sf-settings-subsection-header");
    new import_obsidian20.Setting(interfaceSection).setName("Default view type").setDesc("Choose between list or calendar for the review sidebar").addDropdown((dropdown) => dropdown.addOption("list", "List view").addOption("calendar", "Calendar view").setValue(this.plugin.settings.sidebarViewType).onChange(async (value) => {
      var _a;
      this.plugin.settings.sidebarViewType = value;
      await this.plugin.savePluginData();
      void ((_a = this.plugin.getSidebarView()) == null ? void 0 : _a.refresh());
    }));
    new import_obsidian20.Setting(interfaceSection).setName("Show navigation notifications").setDesc("Display notifications when moving between notes").addToggle((toggle) => toggle.setValue(this.plugin.settings.showNavigationNotifications).onChange(async (value) => {
      this.plugin.settings.showNavigationNotifications = value;
      await this.plugin.savePluginData();
    }));
    new import_obsidian20.Setting(interfaceSection).setName("Review behavior").setHeading().setClass("sf-settings-subsection-header");
    new import_obsidian20.Setting(interfaceSection).setName("Include subfolders").setDesc("When adding a folder to review, include all notes in subfolders").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeSubfolders).onChange(async (value) => {
      this.plugin.settings.includeSubfolders = value;
      await this.plugin.savePluginData();
    }));
    new import_obsidian20.Setting(interfaceSection).setName("Notification time").setDesc("Minutes before due time to notify about upcoming reviews (0 to disable)").addText((text) => text.setValue(this.plugin.settings.notifyBeforeDue.toString()).onChange(async (value) => {
      const numValue = parseInt(value);
      if (!isNaN(numValue) && numValue >= 0) {
    ...
    new import_obsidian20.Setting(interfaceSection).setName(`Reading speed (${WPM})`).setDesc("Words per minute for estimating review time").addSlider((slider) => slider.setLimits(100, 500, 10).setValue(this.plugin.settings.readingSpeed).setDynamicTooltip().onChange(async (value) => {
    ...
    new import_obsidian20.Setting(interfaceSection).setName("Navigation command").setHeading().setClass("sf-settings-subsection-header");
    new import_obsidian20.Setting(interfaceSection).setName("Enable navigation command").setDesc("Execute a command with a slight delay after navigating to the next or previous note.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableNavigationCommands).onChange(async (value) => {
      this.plugin.settings.enableNavigationCommands = value;
      await this.plugin.savePluginData();
      this.display();
    }));
    if (this.plugin.settings.enableNavigationCommands) {
      new import_obsidian20.Setting(interfaceSection).setName("Command to execute").setDesc("Click the button and press the desired hotkey.").addButton((button) => {
        const command = this.plugin.settings.navigationCommand;
      ...
      new import_obsidian20.Setting(interfaceSection).setName("Command execution delay (ms)").setDesc("How long to wait before executing the command after navigation.").addSlider((slider) => slider.setLimits(0, 2e3, 100).setValue(this.plugin.settings.navigationCommandDelay).setDynamicTooltip().onChange(async (value) => {
    ...
    new import_obsidian20.Setting(mcqSection).setName("Enable multiple choice feature").setDesc("Use AI-generated multiple-choice questions to test your knowledge").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableMCQ).onChange(async (value) => {
    ...
    if (this.plugin.settings.enableMCQ) {
      new import_obsidian20.Setting(mcqSection).setName("API configuration").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(mcqSection).setName("API provider").setDesc("Select the API provider for generating MCQs.").addDropdown((dropdown) => {
        dropdown.addOption("openrouter" /* OpenRouter */, "OpenRouter").addOption("openai" /* OpenAI */, "OpenAI").addOption("ollama" /* Ollama */, "Ollama").addOption("gemini" /* Gemini */, "Gemini").addOption("claude" /* Claude */, "Claude").addOption("together" /* Together */, "Together AI").addOption("openai-compatible" /* OpenAICompatible */, "OpenAI Compatible").setValue(this.plugin.settings.mcqApiProvider).onChange(async (value) => {
      ...
      if (provider === "openrouter" /* OpenRouter */) {
        new import_obsidian20.Setting(mcqSection).setName("OpenRouter configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        const apiKeyContainer = mcqSection.createDiv({ cls: "sf-setting-highlight" });
        new import_obsidian20.Setting(apiKeyContainer).setName("API key").setDesc("Required for generating MCQs via OpenRouter.").addText((text) => text.setPlaceholder("Enter your OpenRouter API key").setValue(this.plugin.settings.openRouterApiKey).onChange(async (value) => {
          this.plugin.settings.openRouterApiKey = value;
          await this.plugin.savePluginData();
        }));
        apiKeyContainer.createDiv().setText("Get your API key at https://openrouter.ai/keys");
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc("Model identifier from OpenRouter (e.g., openai/gpt-4.1-mini)").addText((text) => text.setPlaceholder("Enter OpenRouter model identifier").setValue(this.plugin.settings.openRouterModel).onChange(async (value) => {
          this.plugin.settings.openRouterModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "openai" /* OpenAI */) {
        new import_obsidian20.Setting(mcqSection).setName("OpenAI configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API key").setDesc("Your OpenAI API key.").addText((text) => text.setPlaceholder("Enter your OpenAI API key (sk-...)").setValue(this.plugin.settings.openaiApiKey).onChange(async (value) => {
          this.plugin.settings.openaiApiKey = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc("Model name (e.g., gpt-3.5-turbo, gpt-4)").addText((text) => text.setPlaceholder("Enter OpenAI model name").setValue(this.plugin.settings.openaiModel).onChange(async (value) => {
          this.plugin.settings.openaiModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "ollama" /* Ollama */) {
        new import_obsidian20.Setting(mcqSection).setName("Ollama configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API URL").setDesc("URL of your running Ollama instance (e.g., http://localhost:11434)").addText((text) => text.setPlaceholder(`http:${"//"}localhost:11434`).setValue(this.plugin.settings.ollamaApiUrl).onChange(async (value) => {
          this.plugin.settings.ollamaApiUrl = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc(`Name of the ${OLLAMA} model to use (e.g., llama3, mistral)`).addText((text) => text.setPlaceholder(`Enter ${OLLAMA} model name`).setValue(this.plugin.settings.ollamaModel).onChange(async (value) => {
          this.plugin.settings.ollamaModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "gemini" /* Gemini */) {
        new import_obsidian20.Setting(mcqSection).setName("Gemini configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API key").setDesc(`Your Google AI ${GEMINI} ${API} key.`).addText((text) => text.setPlaceholder(`Enter your ${GEMINI} ${API} key`).setValue(this.plugin.settings.geminiApiKey).onChange(async (value) => {
          this.plugin.settings.geminiApiKey = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc("Model name (e.g., gemini-pro)").addText((text) => text.setPlaceholder(`Enter ${GEMINI} model name`).setValue(this.plugin.settings.geminiModel).onChange(async (value) => {
          this.plugin.settings.geminiModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "claude" /* Claude */) {
        new import_obsidian20.Setting(mcqSection).setName("Claude configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API key").setDesc(`Your Anthropic ${CLAUDE} ${API} key.`).addText((text) => text.setPlaceholder(`Enter your ${CLAUDE} ${API} key`).setValue(this.plugin.settings.claudeApiKey).onChange(async (value) => {
          this.plugin.settings.claudeApiKey = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc("Model name (e.g., claude-3-opus-20240229, claude-3-sonnet-20240229)").addText((text) => text.setPlaceholder(`Enter ${CLAUDE} model name`).setValue(this.plugin.settings.claudeModel).onChange(async (value) => {
          this.plugin.settings.claudeModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "together" /* Together */) {
        new import_obsidian20.Setting(mcqSection).setName("Together AI configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API key").setDesc(`Your ${TOGETHER_AI} ${API} key.`).addText((text) => text.setPlaceholder(`Enter your ${TOGETHER_AI} ${API} key`).setValue(this.plugin.settings.togetherApiKey).onChange(async (value) => {
          this.plugin.settings.togetherApiKey = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc("Model identifier from Together AI (e.g., meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8)").addText((text) => text.setPlaceholder(`Enter ${TOGETHER_AI} model identifier`).setValue(this.plugin.settings.togetherModel).onChange(async (value) => {
          this.plugin.settings.togetherModel = value;
          await this.plugin.savePluginData();
        }));
      } else if (provider === "openai-compatible" /* OpenAICompatible */) {
        new import_obsidian20.Setting(mcqSection).setName("OpenAI Compatible configuration").setHeading().setClass("sf-settings-subsection-provider-header");
        new import_obsidian20.Setting(mcqSection).setName("API URL").setDesc(`Base URL of the ${OPENAI_COMPATIBLE} API (e.g., http://localhost:8000/v1)`).addText((text) => text.setPlaceholder("http://localhost:8000/v1").setValue(this.plugin.settings.openaiCompatibleApiUrl).onChange(async (value) => {
          this.plugin.settings.openaiCompatibleApiUrl = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("API key").setDesc(`API key for ${OPENAI_COMPATIBLE} (optional).`).addText((text) => text.setPlaceholder(`Enter ${OPENAI_COMPATIBLE} ${API} key`).setValue(this.plugin.settings.openaiCompatibleApiKey).onChange(async (value) => {
          this.plugin.settings.openaiCompatibleApiKey = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName("Model").setDesc(`Model name to use (e.g., gpt-4, local-model-name)`).addText((text) => text.setPlaceholder(`Enter model name`).setValue(this.plugin.settings.openaiCompatibleModel).onChange(async (value) => {
          this.plugin.settings.openaiCompatibleModel = value;
          await this.plugin.savePluginData();
        }));
      }
      new import_obsidian20.Setting(mcqSection).setName("Question generation (common)").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(mcqSection).setName("Question amount mode").setDesc("How to determine the number of questions per note.").addDropdown((dropdown) => dropdown.addOption("fixed" /* Fixed */, "Fixed number").addOption("wordsPerQuestion" /* WordsPerQuestion */, "Per X words").setValue(this.plugin.settings.mcqQuestionAmountMode).onChange(async (value) => {
        this.plugin.settings.mcqQuestionAmountMode = value;
        await this.plugin.savePluginData();
        this.display();
      }));
      if (this.plugin.settings.mcqQuestionAmountMode === "fixed" /* Fixed */) {
        new import_obsidian20.Setting(mcqSection).setName("Questions per note (fixed)").setDesc("Number of questions to generate for each note.").addSlider((slider) => slider.setLimits(1, 10, 1).setValue(this.plugin.settings.mcqQuestionsPerNote).setDynamicTooltip().onChange(async (value) => {
        ...
        new import_obsidian20.Setting(mcqSection).setName("Words per question target").setDesc("Generate approximately 1 question for every X words in the note.").addText((text) => text.setPlaceholder("100").setValue(this.plugin.settings.mcqWordsPerQuestion.toString()).onChange(async (value) => {
          const numValue = parseInt(value);
          if (!isNaN(numValue) && numValue > 0) {
            this.plugin.settings.mcqWordsPerQuestion = numValue;
            await this.plugin.savePluginData();
          } else {
            new import_obsidian20.Notice("Words per question must be a positive number.");
            text.setValue(this.plugin.settings.mcqWordsPerQuestion.toString());
          }
        }));
      }
      new import_obsidian20.Setting(mcqSection).setName("Choices per question").setDesc("Number of answer choices for each question").addSlider((slider) => slider.setLimits(2, 6, 1).setValue(this.plugin.settings.mcqChoicesPerQuestion).setDynamicTooltip().onChange(async (value) => {
      ...
      new import_obsidian20.Setting(promptTypeContainer).setName("Prompt type").setDesc("Format for question generation").addDropdown((dropdown) => dropdown.addOption("basic", "Basic").addOption("detailed", "Detailed").setValue(this.plugin.settings.mcqPromptType).onChange(async (value) => {
        this.plugin.settings.mcqPromptType = value;
        await this.plugin.savePluginData();
      }));
      const difficultyContainer = mcqFormattingGrid.createDiv();
      new import_obsidian20.Setting(difficultyContainer).setName("Difficulty").setDesc("Complexity level").addDropdown((dropdown) => dropdown.addOption("basic" /* Basic */, "Basic recall").addOption("advanced" /* Advanced */, "Advanced understanding").setValue(this.plugin.settings.mcqDifficulty).onChange(async (value) => {
        this.plugin.settings.mcqDifficulty = value;
        await this.plugin.savePluginData();
      }));
      new import_obsidian20.Setting(mcqSection).setName("Scoring").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(mcqSection).setName("Time deduction amount").setDesc("Score penalty for slow answers (0-1)").addSlider((slider) => slider.setLimits(0, 1, 0.1).setValue(this.plugin.settings.mcqTimeDeductionAmount).setDynamicTooltip().onChange(async (value) => {
        this.plugin.settings.mcqTimeDeductionAmount = value;
        await this.plugin.savePluginData();
      }));
      new import_obsidian20.Setting(mcqSection).setName("Time deduction threshold").setDesc("Apply penalty after this many seconds").addSlider((slider) => slider.setLimits(10, 120, 5).setValue(this.plugin.settings.mcqTimeDeductionSeconds).setDynamicTooltip().onChange(async (value) => {
        this.plugin.settings.mcqTimeDeductionSeconds = value;
        await this.plugin.savePluginData();
      }));
      new import_obsidian20.Setting(mcqSection).setName("Deduct full mark on first failure").setDesc("If enabled, the score for a question will be 0 if the first attempt is incorrect.").addToggle((toggle) => toggle.setValue(this.plugin.settings.mcqDeductFullMarkOnFirstFailure).onChange(async (value) => {
      ...
      new import_obsidian20.Setting(mcqSection).setName("Advanced question behavior").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(mcqSection).setName("Enable question regeneration on rating").setDesc("Automatically regenerate questions for a note if its review rating meets or exceeds a specified value.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableQuestionRegenerationOnRating).onChange(async (value) => {
        this.plugin.settings.enableQuestionRegenerationOnRating = value;
        await this.plugin.savePluginData();
        this.display();
      }));
      if (this.plugin.settings.enableQuestionRegenerationOnRating) {
        new import_obsidian20.Setting(mcqSection).setName(`Minimum rating for regeneration (${SM2})`).setDesc(`For ${SM2}: Regenerate ${MCQS} if review rating (0-5) is this value or higher. (0:Blackout, 5:Perfect)`).addSlider((slider) => slider.setLimits(0, 5, 1).setValue(this.plugin.settings.minSm2RatingForQuestionRegeneration).setDynamicTooltip().onChange(async (value) => {
          this.plugin.settings.minSm2RatingForQuestionRegeneration = value;
          await this.plugin.savePluginData();
        }));
        new import_obsidian20.Setting(mcqSection).setName(`Minimum rating for regeneration (${FSRS})`).setDesc(`For ${FSRS}: Regenerate ${MCQS} if review rating (1-4) is this value or higher. (1:Again, 4:Easy)`).addSlider((slider) => slider.setLimits(1, 4, 1).setValue(this.plugin.settings.minFsrsRatingForQuestionRegeneration).setDynamicTooltip().onChange(async (value) => {
    ...
    new import_obsidian20.Setting(pomodoroSection).setName(`Enable ${POMODORO} timer`).setDesc(`Show the ${POMODORO} timer in the sidebar.`).addToggle((toggle) => toggle.setValue(this.plugin.settings.pomodoroEnabled).onChange(async (value) => {
      var _a, _b;
      this.plugin.settings.pomodoroEnabled = value;
      await this.plugin.savePluginData();
      (_a = this.plugin.pomodoroService) == null ? void 0 : _a.onSettingsChanged();
      void ((_b = this.plugin.getSidebarView()) == null ? void 0 : _b.refresh());
      this.display();
    }));
    if (this.plugin.settings.pomodoroEnabled) {
      new import_obsidian20.Setting(pomodoroSection).setName("Timer durations (minutes)").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(pomodoroSection).setName("Work duration").setDesc("Length of a work session.").addText((text) => text.setPlaceholder("25").setValue(this.plugin.settings.pomodoroWorkDuration.toString()).onChange(async (value) => {
        var _a;
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
      ...
      new import_obsidian20.Setting(pomodoroSection).setName("Short break duration").setDesc("Length of a short break.").addText((text) => text.setPlaceholder("5").setValue(this.plugin.settings.pomodoroShortBreakDuration.toString()).onChange(async (value) => {
        var _a;
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
      ...
      new import_obsidian20.Setting(pomodoroSection).setName("Long break duration").setDesc("Length of a long break.").addText((text) => text.setPlaceholder("15").setValue(this.plugin.settings.pomodoroLongBreakDuration.toString()).onChange(async (value) => {
        var _a;
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
      ...
      new import_obsidian20.Setting(pomodoroSection).setName("Sessions until long break").setDesc("Number of work sessions before a long break starts.").addText((text) => text.setPlaceholder("4").setValue(this.plugin.settings.pomodoroSessionsUntilLongBreak.toString()).onChange(async (value) => {
        var _a;
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
      ...
      new import_obsidian20.Setting(pomodoroSection).setName("Notifications").setHeading().setClass("sf-settings-subsection-header");
      new import_obsidian20.Setting(pomodoroSection).setName("Enable sound notifications").setDesc("Play a sound at the end of each work/break session.").addToggle((toggle) => toggle.setValue(this.plugin.settings.pomodoroSoundEnabled).onChange(async (value) => {
    ...
    if (this.plugin.settings.useCustomDataPath && this.plugin.settings.customDataPath) {
      const customPath = this.plugin.settings.customDataPath.endsWith("/data.json") ? this.plugin.settings.customDataPath : `${this.plugin.settings.customDataPath}/data.json`;
      locationDesc = `Custom: ${customPath}`;
    }
    new import_obsidian20.Setting(dataSection).setName("Current data location").setDesc(locationDesc).addExtraButton((button) => button.setIcon("info").setTooltip(`Current location of your ${SPACEFORGE} data`).onClick(() => {
      const message = this.plugin.settings.useCustomDataPath && this.plugin.settings.customDataPath ? `Your data is stored at: ${this.plugin.settings.customDataPath.endsWith("/data.json") ? this.plugin.settings.customDataPath : `${this.plugin.settings.customDataPath}/data.json`}` : `Your data is stored at: ${defaultPath}`;
    ...
    if (this.plugin.settings.useCustomDataPath) {
      ...
      if (oldFile) {
        new import_obsidian20.Setting(dataSection).setName("Legacy data file found").setDesc(`A data file exists at the default location. You can safely delete it or keep it as a backup.`).addButton((button) => button.setButtonText("Keep as backup").setIcon("save").onClick(() => {
          new import_obsidian20.Notice("Legacy data file kept as backup. You can delete it manually when ready.", 5e3);
        })).addButton((button) => button.setButtonText("Delete legacy file").setIcon("trash").setWarning().onClick(() => {
          new ConfirmationModal(
            this.plugin.app,
            ...
            async () => {
              try {
                ...
                new import_obsidian20.Notice("Legacy data file deleted successfully.", 3e3);
                this.display();
              } catch (error) {
                const msg = error instanceof Error ? error.message : String(error);
                new import_obsidian20.Notice("Failed to delete legacy file: " + msg, 5e3);
              }
            }
          ).open();
        }));
  ...
  renderAboutAlgorithmSection(container, algorithm) {
    container.empty();
    if (algorithm === "sm2") {
      new import_obsidian20.Setting(container).setName(`About modified ${SM2} algorithm`).setHeading();
      container.createEl("p", {
        text: `Spaceforge uses a modified version of the SuperMemo ${SM2} algorithm (1991) which schedules reviews based on how well you recall information. When you rate your recall quality from 0-5, the algorithm adjusts the interval and difficulty (ease factor) accordingly.`
      });
      container.createEl("p", {
      ...
      new import_obsidian20.Setting(container).setName(`About ${FSRS} algorithm`).setHeading();
      container.createEl("p", {
...
var OpenRouterService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.openRouterApiKey) {
      new import_obsidian21.Notice(`${OPENROUTER} ${API} key not set in settings.`);
      return null;
    }
    try {
      new import_obsidian21.Notice(`Generating questions using ${OPENROUTER}...`);
      ...
      if (questions.length === 0) {
        new import_obsidian21.Notice("Failed to generate valid questions. Try again.");
        return null;
      }
      const mcqSet = {
        notePath,
      ...
      new import_obsidian21.Notice("Failed to generate questions. Check console for details.");
      return null;
    }
  }
  /**
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    ...
    const difficulty = settings.mcqDifficulty;
    try {
      const systemPrompt = difficulty === "basic" /* Basic */ ? settings.mcqBasicSystemPrompt : settings.mcqAdvancedSystemPrompt;
      ...
      if (!((_a = data.choices) == null ? void 0 : _a.length) || !((_b = data.choices[0]) == null ? void 0 : _b.message)) {
        throw new Error("Invalid API response format from OpenRouter - missing choices");
      }
      return data.choices[0].message.content;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`OpenRouter API error: ${msg}`);
      throw error;
    }
  }
  /**
   ...
   */
  parseResponse(response, _settings, numQuestionsToGenerate) {
    const questions = [];
    try {
      let questionBlocks = [];
      ...
      new import_obsidian21.Notice(`Error parsing response from ${OPENROUTER}. Try again.`);
      return [];
    }
  }
};
...
var OpenAIService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.openaiApiKey) {
      new import_obsidian22.Notice(`${OPENAI} ${API} key not set in settings.`);
      return null;
    }
    if (!settings.openaiModel) {
      new import_obsidian22.Notice(`${OPENAI} model not set in settings.`);
      return null;
    }
    try {
      new import_obsidian22.Notice(`Generating questions using ${OPENAI}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian22.Notice(`Failed to generate valid questions from ${OPENAI}. Try again.`);
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian22.Notice(`Failed to generate questions with ${OPENAI}. Check console for details.`);
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    ...
    try {
      ...
      if (response.status !== 200) {
        const errorData = (_a = response.json) != null ? _a : { message: response.text };
        throw new Error(`API request failed (${response.status}): ${(_d = (_c = (_b = errorData.error) == null ? void 0 : _b.message) != null ? _c : errorData.message) != null ? _d : "Unknown error"}`);
      }
      const data = response.json;
      if (!((_e = data.choices) == null ? void 0 : _e.length) || !((_g = (_f = data.choices[0]) == null ? void 0 : _f.message) == null ? void 0 : _g.content)) {
        throw new Error("Invalid API response format from OpenAI - missing content");
      }
      return data.choices[0].message.content;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian22.Notice(`OpenAI API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, _settings, numQuestionsToGenerate) {
    const questions = [];
    try {
      const questionBlocks = response.split(/\d+\.\s+/).filter((block) => block.trim().length > 0);
      if (questionBlocks.length === 0) {
        const lines = response.split("\n");
        let currentQuestion = "";
        for (const line of lines) {
      ...
      new import_obsidian22.Notice(`Error parsing response from ${OPENAI}. Try again.`);
      return [];
    }
  }
};
...
var OllamaService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.ollamaApiUrl) {
      new import_obsidian23.Notice(`${OLLAMA} ${API} URL not set in settings.`);
      return null;
    }
    if (!settings.ollamaModel) {
      new import_obsidian23.Notice(`${OLLAMA} model not set in settings.`);
      return null;
    }
    try {
      new import_obsidian23.Notice(`Generating questions using ${OLLAMA}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian23.Notice(`Failed to generate valid questions from ${OLLAMA}. Try again.`);
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian23.Notice(`Failed to generate questions with ${OLLAMA}. Check console for details.`);
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    ...
    try {
      ...
      const data = response.json;
      if (!((_a = data.message) == null ? void 0 : _a.content)) {
        throw new Error("Invalid API response format from Ollama - missing message content");
      }
      return data.message.content;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian23.Notice(`Ollama API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, _settings, numQuestionsToGenerate) {
    ...
    try {
      ...
      new import_obsidian23.Notice(`Error parsing response from ${OLLAMA}. Try again.`);
      return [];
    }
  }
};
...
var GeminiService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.geminiApiKey) {
      new import_obsidian24.Notice(`${GEMINI} ${API} key is not set. Please add it in the settings`);
      return null;
    }
    if (!settings.geminiModel) {
      new import_obsidian24.Notice(`${GEMINI} model is not set. Please add it in the settings`);
      return null;
    }
    try {
      new import_obsidian24.Notice(`Generating ${MCQS} using ${GEMINI}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian24.Notice(`Failed to generate valid ${MCQS} from ${GEMINI}. Please try again`);
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian24.Notice(`Failed to generate ${MCQS} with ${GEMINI}. Please check console for details`);
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
    ...
    const difficulty = settings.mcqDifficulty;
    let basePrompt = "";
    const systemInstruction = difficulty === "basic" /* Basic */ ? settings.mcqBasicSystemPrompt : settings.mcqAdvancedSystemPrompt;
    if (promptType === "basic") {
  ...
  async makeApiRequest(prompt, settings) {
    ...
    try {
      ...
      if (response.status !== 200) {
        ...
        const errorMessage = (_c = (_b = (_a = errorData == null ? void 0 : errorData.error) == null ? void 0 : _a.message) != null ? _b : errorData == null ? void 0 : errorData.message) != null ? _c : "Unknown error";
        throw new Error(`API request failed (${response.status}): ${errorMessage}`);
      }
      const data = response.json;
      if (!((_d = data.candidates) == null ? void 0 : _d.length) || !((_g = (_f = (_e = data.candidates[0]) == null ? void 0 : _e.content) == null ? void 0 : _f.parts) == null ? void 0 : _g.length) || !((_j = (_i = (_h = data.candidates[0]) == null ? void 0 : _h.content) == null ? void 0 : _i.parts[0]) == null ? void 0 : _j.text)) {
        throw new Error("Invalid API response format from Gemini - missing content");
      }
      return data.candidates[0].content.parts[0].text;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian24.Notice(`Gemini API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, _settings, numQuestionsToGenerate) {
    ...
    try {
      ...
      new import_obsidian24.Notice(`Error parsing ${MCQ} response from ${GEMINI}. Please try again`);
      return [];
    }
  }
};
...
var ClaudeService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.claudeApiKey) {
      new import_obsidian25.Notice(`${CLAUDE} ${API} key is not set. Please add it in the settings`);
      return null;
    }
    if (!settings.claudeModel) {
      new import_obsidian25.Notice(`${CLAUDE} model is not set. Please add it in the settings`);
      return null;
    }
    try {
      new import_obsidian25.Notice(`Generating ${MCQS} using ${CLAUDE}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian25.Notice(`Failed to generate valid ${MCQS} from ${CLAUDE}. Please try again`);
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian25.Notice(`Failed to generate ${MCQS} with ${CLAUDE}. Please check console for details`);
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    ...
    try {
      ...
      const data = response.json;
      if (!((_e = data.content) == null ? void 0 : _e.length) || !((_f = data.content[0]) == null ? void 0 : _f.text)) {
        throw new Error("Invalid API response format from Claude - missing content");
      }
      return data.content[0].text;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian25.Notice(`Claude API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, settings, numQuestionsToGenerate) {
    ...
    try {
      ...
      new import_obsidian25.Notice(`Error parsing ${MCQ} response from ${CLAUDE}. Please try again`);
      return [];
    }
  }
};
...
var TogetherService = class {
  ...
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.togetherApiKey) {
      new import_obsidian26.Notice(`${TOGETHER_AI} ${API} key not set in settings.`);
      return null;
    }
    if (!settings.togetherModel) {
      new import_obsidian26.Notice(`${TOGETHER_AI} model not set in settings.`);
      return null;
    }
    try {
      new import_obsidian26.Notice(`Generating questions using ${TOGETHER_AI}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian26.Notice("Failed to generate valid questions. Try again.");
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian26.Notice("Failed to generate questions. Check console for details.");
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian26.Notice(`Together AI API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, settings, numQuestionsToGenerate) {
    ...
    try {
      ...
      new import_obsidian26.Notice(`Error parsing response from ${TOGETHER_AI}. Try again.`);
      return [];
    }
  }
};
...
var OpenAICompatibleService = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  async generateMCQs(notePath, noteContent, settings) {
    if (!settings.openaiCompatibleApiUrl) {
      new import_obsidian27.Notice(`${OPENAI_COMPATIBLE} ${API} URL not set in settings.`);
      return null;
    }
    if (!settings.openaiCompatibleModel) {
      new import_obsidian27.Notice(`${OPENAI_COMPATIBLE} model not set in settings.`);
      return null;
    }
    try {
      new import_obsidian27.Notice(`Generating questions using ${OPENAI_COMPATIBLE}...`);
      let numQuestionsToGenerate;
      if (settings.mcqQuestionAmountMode === "wordsPerQuestion" /* WordsPerQuestion */) {
        const wordCount = noteContent.split(/\s+/).filter(Boolean).length;
        numQuestionsToGenerate = Math.max(1, Math.ceil(wordCount / settings.mcqWordsPerQuestion));
      ...
      const questions = this.parseResponse(response, settings, numQuestionsToGenerate);
      if (questions.length === 0) {
        new import_obsidian27.Notice("Failed to generate valid questions. Try again.");
        return null;
      }
      return {
        notePath,
      ...
      new import_obsidian27.Notice("Failed to generate questions. Check console for details.");
      return null;
    }
  }
  generatePrompt(noteContent, settings, numQuestionsToGenerate) {
...
${noteContent}`;
  ...
  async makeApiRequest(prompt, settings) {
    var _a, _b, _c, _d, _e, _f, _g;
    const apiUrl = settings.openaiCompatibleApiUrl.endsWith("/") ? settings.openaiCompatibleApiUrl.slice(0, -1) : settings.openaiCompatibleApiUrl;
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian27.Notice(`OpenAI Compatible API error: ${msg}`);
      throw error;
    }
  }
  parseResponse(response, settings, numQuestionsToGenerate) {
    ...
    try {
      ...
      new import_obsidian27.Notice(`Error parsing response from ${OPENAI_COMPATIBLE}. Try again.`);
      return [];
    }
  }
};
  ...
  mapFsrsLibRatingToTsFsrsRating(rating) {
    switch (rating) {
      ...
      case 4 /* Easy */:
        return l.Easy;
      default:
        throw new Error(`Unknown FsrsRating: ${String(rating)}`);
    }
  }
...
var ReviewScheduleService = class {
   ...
   */
  scheduleNoteForReview(path, daysFromNow = 0) {
    const file = this.plugin.app.vault.getAbstractFileByPath(path);
    if (!file || !(file instanceof import_obsidian28.TFile) || file.extension !== "md") {
      new import_obsidian28.Notice("Only Markdown files can be added to review.");
      return;
    }
    const now = Date.now();
    ...
    new import_obsidian28.Notice(`Note added to review.`);
  }
  /**
   ...
   */
  recordReview(path, response, isSkipped = false, currentReviewDate) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j;
    ...
    if (schedule.schedulingAlgorithm === "fsrs") {
      ...
      if (response === 5 /* PerfectRecall */) {
        actualFsrsRating = 4 /* Easy */;
      } else if (response === 4 /* CorrectWithHesitation */) {
        actualFsrsRating = 3 /* Good */;
      } else if (response === 3 /* CorrectWithDifficulty */) {
        actualFsrsRating = 2 /* Hard */;
      } else if (response === 1 /* IncorrectResponse */) {
      ...
      const qualityRating = toSM2Quality(response);
   ...
   * @param currentReviewDate Optional timestamp for the current review date (simulated or actual)
   */
  skipNote(path, response = 3 /* CorrectWithDifficulty */, currentReviewDate) {
    const schedule = this.schedules[path];
    if (!schedule)
      return;
    const effectiveReviewDate = currentReviewDate || Date.now();
  ...
  postponeNote(path, days = 1) {
    ...
    new import_obsidian28.Notice(`Review postponed for ${days} day${days !== 1 ? "s" : ""}.`);
  }
  /**
   * Advance a note's review by one day, if eligible.
   *
  ...
  clearAllSchedules() {
    this.schedules = {};
    this.customNoteOrder = [];
    new import_obsidian28.Notice("All review schedules cleared.");
    if (this.plugin.events) {
      this.plugin.events.emit("sidebar-update");
    }
    if (this.plugin.reviewController) {
   ...
   */
  scheduleNotesInOrder(paths, daysFromNow = 0) {
    let count = 0;
    for (const path of paths) {
      const file = this.plugin.app.vault.getAbstractFileByPath(path);
      if (!file || !(file instanceof import_obsidian28.TFile) || file.extension !== "md" || this.schedules[path]) {
        continue;
      }
      const now = Date.now();
      const todayUTCStart = DateUtils.startOfUTCDay(new Date(now));
   ...
   */
  updateCustomNoteOrder(newOrder) {
    const uniqueValidPaths = Array.from(new Set(newOrder)).filter((path) => this.schedules[path] !== void 0);
...
var ReviewSessionService = class {
  ...
  async createReviewSession(folderPath, name) {
    ...
    if (!folder || !(folder instanceof import_obsidian29.TFolder)) {
      new import_obsidian29.Notice("Invalid folder for review session");
      return null;
    }
    try {
      ...
      new import_obsidian29.Notice("Failed to create review session");
      return null;
    }
  }
  /**
  ...
  advanceActiveSession() {
    ...
    if (isSessionComplete(updatedSession)) {
      ...
      this.reviewSessions.activeSessionId = null;
      new import_obsidian29.Notice(`Completed review session: ${updatedSession.name}`);
    }
    if (this.plugin.events) {
      this.plugin.events.emit("sidebar-update");
    }
...
// services/recurring-notes-service.ts
var RecurringNotesService = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  /**
   ...
   */
  async convertToRecurringNote(path, interval, endDate = null) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    if (!schedule) {
   ...
   * @param path Path to the note file
   */
  async scheduleNextRecurrence(path) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    ...
    const nextOccurrenceDate = nextReviewDate + schedule.recurrenceInterval * 24 * 60 * 60 * 1e3;
    ...
    schedule.lastReviewDate = nextReviewDate;
    await this.plugin.savePluginData();
  }
  /**
   ...
   * @param path Path to the note file
   */
  async convertToRegularNote(path) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    ...
    schedule.isRecurring = false;
    schedule.recurrenceInterval = void 0;
    schedule.recurrenceEndDate = void 0;
   ...
   */
  isNoteRecurring(path) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    return !!(schedule == null ? void 0 : schedule.isRecurring);
  }
  /**
   ...
   */
  getRecurrenceInfo(path) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    if (!(schedule == null ? void 0 : schedule.isRecurring) || !schedule.recurrenceInterval) {
      return null;
    }
    return {
   ...
   */
  async processAllRecurringNotes() {
   ...
   */
  async updateRecurrenceInterval(path, newInterval) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    if (!(schedule == null ? void 0 : schedule.isRecurring)) {
      return;
    }
   ...
   */
  async updateRecurrenceEndDate(path, newEndDate) {
    const schedule = this.plugin.reviewScheduleService.schedules[path];
    if (!(schedule == null ? void 0 : schedule.isRecurring)) {
      return;
    }
...
// main.ts
var SpaceforgePlugin = class extends import_obsidian30.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (viewWithFile == null ? void 0 : viewWithFile.file) {
          ...
          if (selectedFile instanceof import_obsidian30.TFile && selectedFile.extension === "md") {
            await this.reviewScheduleService.scheduleNoteForReview(selectedFile.path);
            await this.savePluginData();
            new import_obsidian30.Notice(`Added "${selectedFile.path}" to review schedule.`);
          } else {
            new import_obsidian30.Notice("Selected item is not a Markdown file.");
          }
        } else {
          new import_obsidian30.Notice("No file selected in file explorer");
        }
      }
    });
    ...
    this.registerEvent(this.app.vault.on("delete", async (file) => {
      ...
      void ((_a2 = this.getSidebarView()) == null ? void 0 : _a2.refresh());
    }));
  ...
  _getEffectiveDataPath() {
    ...
    if ((_a = this.settings) == null ? void 0 : _a.useCustomDataPath) {
      ...
      if (relativePath && relativePath !== "") {
        ...
        absolutePath = (0, import_obsidian30.normalizePath)(absolutePath);
        return absolutePath;
      } else {
        return null;
      }
  ...
  async loadPluginData() {
    ...
    let rawLoadedData;
    const effectivePath = this._getEffectiveDataPath();
    ...
    try {
      if (effectivePath) {
        ...
        if (file instanceof import_obsidian30.TFile) {
          ...
          new import_obsidian30.Notice(`Spaceforge: Loaded data from custom path: ${effectivePath}`, 3e3);
        } else {
          const oldFile = this.app.vault.getAbstractFileByPath(defaultPluginDataPath);
          if (oldFile instanceof import_obsidian30.TFile) {
            new import_obsidian30.Notice(`Spaceforge: Custom data file not found at ${effectivePath}. Attempting to migrate from default location.`, 5e3);
            try {
          ...
          if (!rawLoadedData) {
            new import_obsidian30.Notice(`Spaceforge: No data file found at custom path ${effectivePath}. New data file will be created on save.`, 3e3);
          }
        }
      } else {
      ...
      try {
        ...
        if (backupData && typeof backupData === "string") {
          ...
          if (parsedBackup.reviewData) {
            ...
            new import_obsidian30.Notice("Recovered data from backup", 5e3);
          } else {
        ...
        this.settings = { ...DEFAULT_SETTINGS };
        this.pluginState = { ...DEFAULT_PLUGIN_STATE_DATA };
        new import_obsidian30.Notice("Error loading data, using defaults to prevent crash", 5e3);
      }
      this.reviewScheduleService.schedules = this.pluginState.schedules || {};
      this.reviewHistoryService.history = this.pluginState.history || [];
      this.reviewSessionService.reviewSessions = this.pluginState.reviewSessions || { sessions: {}, activeSessionId: null };
  ...
  async savePluginData() {
    ...
    try {
      ...
      if (effectiveSavePath) {
        try {
          ...
          if (dirPathOnly && !this.app.vault.getAbstractFileByPath(dirPathOnly)) {
            ...
            new import_obsidian30.Notice(`Created directory for custom data: ${dirPathOnly}`, 3e3);
          }
          ...
          const oldFile = this.app.vault.getAbstractFileByPath(defaultPluginDataPath);
          if (oldFile instanceof import_obsidian30.TFile) {
            new import_obsidian30.Notice(`Successfully saved to custom path, original data file kept as backup for safety`, 5e3);
          }
        } catch (writeError) {
          const msg = writeError instanceof Error ? writeError.message : String(writeError);
          new import_obsidian30.Notice(`Error saving data to custom path ${effectiveSavePath}: ${msg}. Falling back to default path for this save.`, 1e4);
          try {
            await this.saveData(dataToSave);
            new import_obsidian30.Notice(`Data saved to default plugin folder due to error with custom path`, 5e3);
          } catch (e) {
            new import_obsidian30.Notice(`Critical error: failed to save data to both custom and default locations`, 1e4);
          }
        }
      } else {
        await this.saveData(dataToSave);
      ...
      new import_obsidian30.Notice("Error saving data. Check console for details", 5e3);
    }
  }
  async activateSidebarView() {
    ...
    if (existingLeaves.length > 0) {
      ...
      if (leaf) {
        ...
        new import_obsidian30.Notice("Could not open sidebar view");
      }
    }
  }
  addCommands() {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (activeFile && activeFile instanceof import_obsidian30.TFile && activeFile.extension === "md") {
          void this.reviewController.reviewNote(activeFile.path);
        } else {
          new import_obsidian30.Notice("No active Markdown file to review");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (activeFile && activeFile instanceof import_obsidian30.TFile && activeFile.extension === "md") {
          ...
          await this.savePluginData();
          new import_obsidian30.Notice(`Added "${activeFile.path}" to review schedule`);
        } else {
          new import_obsidian30.Notice("No active Markdown file to add to review");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        const activeFile = this.app.workspace.getActiveFile();
        if (activeFile && activeFile.parent && activeFile.parent instanceof import_obsidian30.TFolder) {
          const folder = activeFile.parent;
          await this.contextMenuHandler.addFolderToReview(folder);
        } else {
          new import_obsidian30.Notice("Could not determine the current note's folder, no active file, or parent is not a folder");
        }
      }
    });
  }
  ...
  initializeMCQComponents() {
    ...
    if (this.settings.enableMCQ) {
      ...
      if (this.mcqGenerationService) {
        ...
        new import_obsidian30.Notice("Generation service could not be initialized. Check API provider settings in settings");
      }
    }
  }
    ...
    switch (this.settings.mcqApiProvider) {
      case "openrouter" /* OpenRouter */:
        if (!this.settings.openRouterApiKey) {
          new import_obsidian30.Notice("OpenRouter API key is not set in settings");
          return void 0;
        }
        return new OpenRouterService(this);
      case "openai" /* OpenAI */:
        if (!this.settings.openaiApiKey) {
          new import_obsidian30.Notice("OpenAI API key is not set in settings");
          return void 0;
        }
        return new OpenAIService(this);
      case "ollama" /* Ollama */:
        if (!this.settings.ollamaApiUrl || !this.settings.ollamaModel) {
          new import_obsidian30.Notice("Ollama API URL or model is not set in settings");
          return void 0;
        }
        return new OllamaService(this);
      case "gemini" /* Gemini */:
        if (!this.settings.geminiApiKey) {
          new import_obsidian30.Notice("Gemini API key is not set in settings");
          return void 0;
        }
        return new GeminiService(this);
      case "claude" /* Claude */:
        if (!this.settings.claudeApiKey || !this.settings.claudeModel) {
          new import_obsidian30.Notice("Claude API key or model is not set in settings");
          return void 0;
        }
        return new ClaudeService(this);
      case "together" /* Together */:
        if (!this.settings.togetherApiKey || !this.settings.togetherModel) {
          new import_obsidian30.Notice("Together AI API key or model is not set in settings");
          return void 0;
        }
      ...
      case "openai-compatible" /* OpenAICompatible */:
        if (!this.settings.openaiCompatibleApiUrl || !this.settings.openaiCompatibleModel) {
          new import_obsidian30.Notice("OpenAI Compatible API URL or model is not set in settings");
          return void 0;
        }
        return new OpenAICompatibleService(this);
      default:
        new import_obsidian30.Notice(`Unsupported MCQ API provider selected: ${String(this.settings.mcqApiProvider)}`);
        return void 0;
    }
  }
  checkForDueNotes() {
